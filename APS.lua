...
Main2:Toggle{
        ...
        while AutoLeave12 do
	        task.wait(3)
            if plyr.PlayerGui.UI.HUD.RaidEvolved.Room.Text == "Wave "..room12 then
                local args = {
                    [1] = "Teleport",
                    [2] = "Spawn",
                    [3] = "Desert Piece"
...
local AutoCrystalDefense
Main2:Toggle{
    Name = "AutoCrystalDefense",
    StartingState = false,
    Description = nil,
    Callback = function(state)
        ...
        while AutoCrystalDefense do
            ...
            local mob
            for _,v in pairs(workspace.Server.ProtectCrystal.Enemies:GetChildren()) do
                if v:GetAttributes()["Health"] > 0 then
                    mob = v
                    break
                end
...
local room4
Main2:Textbox{
	Name = "AutoLeave Room Crystal Defense",
	Callback = function(text)
        room4 = text
    end
}
local AutoLeave4
Main2:Toggle{
    Name = "AutoLeave Crystal Defense",
    StartingState = false,
    Description = nil,
    Callback = function(state)
        AutoLeave4 = state
